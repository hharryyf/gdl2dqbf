...
aspify(T, Out) :-
	...
	write(Out, 'timedomain(1..'), write(Out, T), writeln(Out, ').'),
	aspify(Out).
...
aspify_atom(Atom, AspAtom, T, DomainDeclaration) :-
	...
	( P = true ->
	  ...
	  P = legal ->
	    ...
	    DomainDeclaration = [timedomain(1) | DomainDecl],		% timedomain(T) stands for the domain of time points
	    append(Args, [T], TArgs),
	    AspAtom =.. [legal | Args]
	  ;
	  P = next ->
	    aspify_arguments(P, Args, DomainDecl),
	    DomainDeclaration = [not terminal | DomainDecl],	% special time domain for "next"-clauses
	    append(Args, [next], TArgs),				% next(T) will be replaced by T+1
	    AspAtom =.. [next | Args]
	  ;
	  P = terminal ->
	    DomainDeclaration = [timedomain(1)],	
	    AspAtom =.. [terminal]
	  ;
	  P = goal ->
	    aspify_arguments(P, Args, DomainDecl),
	    DomainDeclaration = [timedomain(1) | DomainDecl],
	    append(Args, [T], TArgs),
	    AspAtom =.. [goal | Args]
	  ;
	  dynamic_feature(P) ->
